...
//	       web browser will take hit&miss value from process //
///////////////////////////////////////////////////////////////////
...
int main(int argc, char * argv[]){
	...
	if(pid == 0){//in sub process
		...
		tok = strtok(tmp, " ");								      //take method from tmp(buf)
		strcpy(method, tok);
		if(strcmp(method, "GET")==0){
			printf("[%s : %d] client was connected\n", inet_ntoa(inet_client_address), client_addr.sin_port);//string about connect
			puts("======================================================");
